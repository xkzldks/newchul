...
document.addEventListener('DOMContentLoaded', function(){
  document.getElementById('download').addEventListener('click', function(){
    ...
    na = year + month + day +'.csv'
    saveCSV(na); // CSV파일 다운로드 함수 호출
    return false;
  })
});
            ...
            success: function (response) {
                ...
                if (date.length != 0){
                    for (let i = 0; i < date.length; i++){
                         var txt = "<th class = 'thz' style = 'z-index: 1;'>"+date[i]['yearDate']+"</th>"
    ...
    document.getElementById("graphPanel").innerHTML = "";
               $.ajax({
                    ...
                    success: function (response) {
                        ...
                        for (let i = 0; i < chul.length; i++){
                            if (chul.length != 0){
                                if ((chul[i][2]/48*100).toFixed(1)> 70){
                                    console.log(chul[i][0] + (chul[i][2]/48*100).toFixed(1));
                                    var txt = "<div class='zt-skill-bar'><div data-width='"+chul[i][2]+"' style = 'white-space : nowrap; width:"+(chul[i][2]/48*100)+"%;'>"+(i+1)+". "+chul[i][0]+" | "+chul[i][2]+"일 | "+(chul[i][2]/48*100).toFixed(1)+"%</div></div>"
                                    document.getElementById("graphPanel").innerHTML += txt;
                                }
                                else{
                                    var txt = "<div class='xt-skill-bar'><div data-width='"+chul[i][2]+"' style = 'white-space : nowrap; width:"+(chul[i][2]/48*100)+"%;'>"+(i+1)+". "+chul[i][0]+" | "+chul[i][2]+"일 | "+(chul[i][2]/48*100).toFixed(1)+"%</div></div>"
                                    document.getElementById("graphPanel").innerHTML += txt;
                                }
                            }
                            else{
